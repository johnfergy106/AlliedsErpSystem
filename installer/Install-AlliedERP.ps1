...
try {
  ...
  $detectedHost = Get-LanIPv4
  $publicHost = Read-Default "Network address employees will use (blank for auto-detect at launch)" $detectedHost
...
"@
  ...
  if ($publicHost) {
    ...
    Write-Host "Network URL will be shown in the Allied ERP launcher window."
  }
