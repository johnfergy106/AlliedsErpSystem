...
3. Use the network URL shown in the launcher window from another computer on the same network.
4. Keep the launcher window open while using the ERP.
5. Other computers must use the server computer's IP address, not their own localhost address.
...
- Deleted customers, sales orders, products, and users are retained as delete markers so stale browser sessions cannot restore them.
...
- New products can also be added directly from the new sales order screen.
